...
void InterfaceConsole::choisirAction(Partie& partie, Joueur& joueur) const
{
    ...
    case 5:
        ...
        if (sauvegarder == 'o') {
            partie.saveToFile("partie_sauvegardee.txt");
...
void InterfaceConsole::lancerPartie(Partie& partie) const {
    std::ifstream saveFile("partie_sauvegardee.txt");
    if (saveFile) {
        char choix;
        std::cout << "Une sauvegarde existe. Voulez-vous la charger ? (o/n) : ";
        std::cin >> choix;

        if (choix == 'o') {
            try {
                partie.loadFromFile("partie_sauvegardee.txt");
                std::cout << "Partie chargee avec succes.\n";
                saveFile.close();
                return;
            } catch (const std::exception& e) {
                std::cerr << "Erreur lors du chargement de la partie : " << e.what() << "\n";
            }
        }
        saveFile.close();
    }
...
void InterfaceConsole::finirPartie(Partie& partie, Joueur& gagnant) const {
    ...
    if (partie.getFin() == TypeFin::Egalite) {
        std::cout << "Egalite parfaite !\n";
    } else {
        std::cout << "Victoire de " << gagnant.getNom() << " (" << gagnant.getCouleur() << ")\n";
    }

    char choix;
    std::cout << "Voulez-vous recommencer une nouvelle partie ? (o/n) : ";
    std::cin >> choix;
...
bool InterfaceConsole::proposerRechargerPartie(Partie& partie) const {
    std::ifstream saveFile("partie_sauvegardee.txt");
    if (!saveFile.is_open()) {
        return false; 
    }
    ...
    char choix;
    std::cout << "Une partie sauvegardee a ete trouvee. Voulez-vous la charger ? (o/n): ";
    std::cin >> choix;

    if (choix == 'o') {
        try {
            partie.loadFromFile("partie_sauvegardee.txt");
            std::cout << "Partie rechargee avec succes.\n";
            return true;
        } catch (const std::exception& e) {
            std::cerr << "Erreur lors du chargement de la partie : " << e.what() << "\n";
        }
    ...
    return false; // Default to starting a new game
}
